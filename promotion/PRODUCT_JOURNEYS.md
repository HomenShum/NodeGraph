...
- **Evidence:** Step 1 verified — `node mcp/client-demo.mjs` exit 0, output
  ...
  **not driven** — the viewer half of this journey has no browser capture in this
  baseline, so the journey is half-proved.
...
- **Recovery** — covered in part by **J5**, and only outside the browser. NodeGraph
  ...
  In the demo that path is unreachable: no scenario produces a rejection and the
  renderer has no error surface at all, which is why condition 5 is FAIL rather
  than covered by a journey.
