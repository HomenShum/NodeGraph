...
Rows carrying an iteration note were re-measured after that iteration landed;
every other row is still the baseline reading and has not been re-run.

| # | Condition | Status | Evidence / reason |
|---|-----------|--------|-------------------|
| 1 | Journeys succeed end-to-end in a real browser | FAIL | J1, J2, J3 drove clean (`01-desktop-default.png`, `18-unknown-readout.png`, `09-selection-readout.png`, `20-drag-after.png`). J4 has no run command anywhere in the repo — `examples/compose/` is a headline README proof with no script in either package.json; it only rendered after I hand-wrote a static server. **Wave 3: J4 now runs from `npm run example:compose` and was re-driven in headless Chromium — 46 entities, 94 relationships, 0 console errors, `promotion/evidence/wave3/`.** J5's browser half (`mcp/viewer/`) is still not driven, so this row stays FAIL. |
| 2 | No critical or major usability defect open | FAIL | 6 of 7 defects still open with reproductions in [PROMOTION_LOG.md](PROMOTION_LOG.md); D1 (`npm run verify:demo` exits 1 on a fresh clone) and D2 (both READMEs tell the reader to press a control that no longer exists) are major. D3 (the three trust classes drawn as the same line) closed in iteration 1. **Wave 3 closed D1, D2 and D4**; three remain — D5 and D7 major, D6 minor. D7 (the graph is unreachable by keyboard, and the node readout is where the trust grammar is disclosed) is the highest-impact one left. |
...
| 11 | Tests and build green | PASS | Green: root `npm test` 14 passed (exit 0), root `npm run typecheck` (exit 0), root `npm run build` (exit 0), render `npm test` **11 passed** (exit 0, was 7 — iteration 1 added `render/tests/edge-grammar.test.mjs`), render `npm run typecheck` (exit 0). Was red at baseline and after iteration 1: render `npm run verify:demo` exited 1, `demo proof failed: {"htmlStatus":200,"title":true,"liveButton":false,...}`. That command is listed in `render/README.md`'s own verification block. **Wave 3: now green.** `verify:demo` exits 0 on six consecutive runs (the gate asserted a control the ten-scenario gallery had deleted, and then two timing defects behind it). Also green: `npm run docs:check`, `npm run proof:edge-grammar` 6/6 pairs. Re-measured 2026-08-13. |
| 12 | Verified in the rendered app, not inferred from code | PASS | Iteration 1 is the only change this loop has made, and it was reproduced and re-proved in headless Chromium against the running demo, not read off the source. Defect state before: `promotion/evidence/edge-grammar/before/` (verdict FAIL, exit 1). After: `.../after/` (verdict PASS, exit 0, 6/6 pairs). Producer `promotion/scripts/prove-edge-grammar.mjs` is committed, spawns its own server, and regenerates both halves — see `promotion/evidence/edge-grammar/README.md`. |

**Status: NOT PROMOTED** — 4/12 PASS (iteration 1: condition 12; wave 3: condition 11).
