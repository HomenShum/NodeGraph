...
| 11 | Tests and build green | FAIL | Green: root `npm test` 14 passed (exit 0), root `npm run typecheck` (exit 0), root `npm run build` (exit 0), render `npm test` **11 passed** (exit 0, was 7 — iteration 1 added `render/tests/edge-grammar.test.mjs`), render `npm run typecheck` (exit 0). Red: render `npm run verify:demo` **exit 1** — `demo proof failed: {"htmlStatus":200,"title":true,"liveButton":false,...}`, the identical signature as baseline. That command is listed in `render/README.md`'s own verification block. |
...
**Status: NOT PROMOTED** — 3/12 PASS (iteration 1: condition 12).
